...
public class dreamloLeaderBoard : MonoBehaviour
{
    ...
    public string privateCode = "HUOcKFehnUqMukpzOPjFIgEHkzWFqRVUaU-EUaeGv6hg";
    public string publicCode = "5bf40713b6397e00e07a6b2b";
    ...
    public struct Score
    {
        ...
        public Score(string piped)
        {
            var fields = piped.Split('|');
            playerName = fields[0];
    ...
    public void LoadScoresTo(UnityAction<IEnumerable<Score>> action)
    {
        if (TooManyRequests()) return;
        StartCoroutine(GetScoresTo(action));
    }

    private IEnumerator GetScoresTo(UnityAction<IEnumerable<Score>> action)
    {
        var www = new WWW(dreamloWebserviceURL + publicCode + "/pipe");
        yield return www;
        var scoreEntries = www.text.Split(new[] {'\n'}, StringSplitOptions.RemoveEmptyEntries);
        var highScore = scoreEntries.Select(scoreEntry => new Score(scoreEntry));
        action.Invoke(highScore);
    }

    public void LoadSingleScore(string playerName, UnityAction<Score> callback)
    {
        if (TooManyRequests()) return;
        StartCoroutine(GetSingleScoreTo(playerName, callback));
    }

    private IEnumerator GetSingleScoreTo(string playerName, UnityAction<Score> callback)
    {
        var www = new WWW(dreamloWebserviceURL + publicCode + "/pipe-get/" + WWW.EscapeURL(playerName));
        yield return www;
        var scoreEntries = www.text.Split(new[] {'\n'}, StringSplitOptions.RemoveEmptyEntries);
        var highScore = scoreEntries.Select(scoreEntry => new Score(scoreEntry));
        callback(highScore.FirstOrDefault());
    }
}